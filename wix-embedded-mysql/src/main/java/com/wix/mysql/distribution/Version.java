...
public enum Version implements IVersion {
  ...
  v8_0_21_twttr("8.0", 21, MacOsVersion.v10_15, TXZ, true, Platform.Linux, Platform.Windows, Platform.OS_X),
  v8_latest(v8_0_17);
  ...
  private final Boolean twitterArtifact;

  Version(String majorVersion, int minorVersion, MacOsVersion macOsVersion, ArchiveType linuxArchiveType, boolean twitterArtifact, Platform... supportedPlatforms) {
    ...
    this.twitterArtifact = twitterArtifact;
  }
  ...
  Version(String majorVersion, int minorVersion, MacOsVersion macOsVersion, ArchiveType linuxArchiveType, Platform... supportedPlatforms) {
    this(majorVersion, minorVersion, macOsVersion, linuxArchiveType, false, supportedPlatforms);
  }
  ...
  Version(Version other) {
    ...
    this.twitterArtifact = other.twitterArtifact;
  }
  ...
  public String asInDownloadPath() {
    ...
    String suffix = twitterArtifact ? format("-%s-%s", arch(), version()) : format("-%s", arch());
    ...
    switch (currentPlatform()) {
      ...
      case OS_X:
      case Linux:
        return format("/%s/%s%s", path(), fileName(), suffix);
      default:
        throw new UnsupportedPlatformException("Unrecognized platform, currently not supported");
    }
  }
  ...
  private String path() {
    return twitterArtifact ? format("%s-%s/%s", fileName(), arch(), version()) : format("MySQL-%s", majorVersion);
  }
